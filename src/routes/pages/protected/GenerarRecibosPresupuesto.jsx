...
import { ModalEditarPresupuesto } from "../../../components/generarRecibos/ModalEditarPresupuesto";

export const GenerarRecibosPresupuesto = () => {
  //   const { ingresos } = useIngresosContext();

  const { openModalEditarTwo } = usePresupuestosContext();
  ...
  const obtenerIngresoRangoFechas = async (fechaInicio, fechaFin) => {
    try {
      ...
      const response = await client.post("/presupuesto/rango-fechas", {
        fechaInicio,
        fechaFin,
      });
    ...
    <section className="px-10 py-16 w-full h-full flex flex-col gap-5">
      ...
      <div className="flex">
        <p className="text-slate-700 text-base border-b-[3px] border-indigo-500">
          Buscar Presupuestos asignados de mes a mes.
        </p>
      </div>
      <div className="mt-10">
      ...
      <div className="flex gap-5">
        <div className="text-sm text-slate-700 font-normal flex gap-3 items-center uppercase">
