...
import { editarTipo, obtenerUnicoTipo } from "../../api/tipos";
import { useForm } from "react-hook-form";

export const ModalEditarTipo = ({ obtenerId, isOpenEdit, closeEdit }) => {
  const {
    register,
    setValue,
    handleSubmit,
  ...
  const { tipos, setTipos } = useTipoContext();
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const tipoExistenteIndex = tipos.findIndex(
                ...
                >
                  <div className="flex flex-col gap-1">
                    <label htmlFor="" className="text-slate-600">
                      Ingresa el detalle del tipo a editar
                    </label>
                    <input
